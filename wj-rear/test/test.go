...
func Mean() {
	...
	for i := 0; i < totalNum; i++ {
		...
		t := (data[i] - (float64(dataMax)+float64(dataMin))/2) / ((float64(dataMax) - float64(dataMin)) / 2)
		p := (1 + t) / 2
		if p < rand.Float64() {
			RelData[i] = -1
		} else {
	...
	fmt.Println(res)
	checkValue := float64(res[0]*(-1)+res[1]*1)/float64(totalNum)*((dataMax-dataMin)/2) + (dataMax+dataMin)/2
